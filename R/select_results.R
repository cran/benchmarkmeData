...
#' @export
#' @examples
#' select_results(test_group = "prog", blas_optimize = NULL)
select_results = function(
  ...
  results = results |>
    ...
    filter(!is.na(time)) |>
    group_by(.data$id, .data$cpu, .data$date, .data$sysname, .data$blas_optimize, .data$test_group, .data$ram, .data$cores) |>
    summarise(time = sum(time)) |>
    ungroup()

  results = results |>
    arrange(time) |>
    mutate(rank = seq_along(time)) |>
